...
//metakeys
    protected $wordLengthMin = 4;
    protected $wordOccuredMin = 2;
    protected $countWord = 3;
//minimum word length for inclusion into the 2 word
//phrase metakeys
    protected $word2WordPhraseLengthMin = 6;
    protected $phrase2WordLengthMinOccur = 2;
    protected $phrase2WordLengthMin = 3;
    protected $count2Word = 2;
//minimum word length for inclusion into the 3 word
//phrase metakeys
    protected $word3WordPhraseLengthMin=10;
    protected $phrase3WordLengthMinOccur = 2;
    protected $phrase3WordLengthMin = 3;
    protected $count3Word = 1;
//minimum phrase length for inclusion into the 2 word
    function __construct($string,$encoding = 'utf-8',$params = array()) {
//get parameters
        $this->encoding = $encoding;
        mb_internal_encoding($encoding);
        $this->contents = $this->replace_chars($string);

// single word
        if(isset($params['min_word_length'])) $this->wordLengthMin = $params['min_word_length'];
        if(isset($params['min_word_occur'])) $this->wordOccuredMin = $params['min_word_occur'];
        if(isset($params['count_word'])) $this->countWord = $params['count_word'];

// 2 word phrase
        if(isset($params['min_2words_length'])) $this->word2WordPhraseLengthMin = $params['min_2words_length'];
        if(isset($params['min_2words_phrase_length'])) $this->phrase2WordLengthMin = $params['min_2words_phrase_length'];
        if(isset($params['min_2words_phrase_occur'])) $this->phrase2WordLengthMinOccur = $params['min_2words_phrase_occur'];
        if(isset($params['count_2word'])) $this->count2Word = $params['count_2word'];

// 3 word phrase
        if(isset($params['min_3words_length'])) $this->word3WordPhraseLengthMin = $params['min_3words_length'];
        if(isset($params['min_3words_phrase_length'])) $this->phrase3WordLengthMin = $params['min_3words_phrase_length'];
        if(isset($params['min_3words_phrase_occur'])) $this->phrase3WordLengthMinOccur = $params['min_3words_phrase_occur'];
        if(isset($params['count_3word'])) $this->count3Word = $params['count_3word'];
...
//lowest.
        ...
        $imploded = $this->implode(", ", array_slice($occur_filtered,0,$this->countWord));
...
//sort the words from highest count to the lowest.
        ...
        $imploded = $this->implode(", ", array_slice($occur_filtered,0,$this->count2Word));
//release unused variables
        unset($y);
        unset($x);
...
//lowest.
        ...
        $imploded = $this->implode(", ", array_slice($occur_filtered,0,$this->count3Word));
//release unused variables
        unset($a);
        unset($b);
