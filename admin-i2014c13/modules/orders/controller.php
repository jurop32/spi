...
class orders extends maincontroller{
    ...
    public function getItems(){
        if($_SESSION[PAGE_SESSIONID]['privileges']['orders'][0] == '0') die($this->text('dont_have_permission'));
        ...
        if (count($orders) > 0) {
            ...
            if($_SESSION[PAGE_SESSIONID]['privileges']['orders'][2] == '1' || $_SESSION[PAGE_SESSIONID]['privileges']['orders'][3] == '1'){
            ...
            foreach($orders as $order){
                ...
                $markup .= '<td><button class="ui-icon-refresh notext button" onclick="acquireItem('.$order['id'].');"'.(($order['ukoncene'] == '1' || strtotime('now') < strtotime($order['datum_platnosti']))?' disabled="disabled"':'').'>'.$this->text('t9').'</button></td>';
                if($_SESSION[PAGE_SESSIONID]['privileges']['orders'][2] == '1' || $_SESSION[PAGE_SESSIONID]['privileges']['orders'][3] == '1'){
                    $markup .= '<td><button class="ui-icon-pencil notext button" onclick="editItem('.$order['id'].');"'.(($_SESSION[PAGE_SESSIONID]['privileges']['orders'][3] == '0' && $order['vlastnik'] != $_SESSION[PAGE_SESSIONID]['id'])?' disabled="disabled"':'').'>'.$this->text('edit').'</button></td>';
                    $markup .= '<td><button class="ui-icon-trash notext button" onclick="confirmDeleteItem('.$order['id'].');"'.(($order['ukoncene'] == '1' || ($_SESSION[PAGE_SESSIONID]['privileges']['orders'][3] == '0' && $order['vlastnik'] != $_SESSION[PAGE_SESSIONID]['id']))?' disabled="disabled"':'').'>'.$this->text('delete').'</button></td>';
                }
                $markup .= '</tr>';
            }
            $markup .= '</table>';
            ...
            return array('state'=>'ok','data'=> '<p>'.$this->text('noorders').'</p>');
        }
    }

    /*
     * returning the selected item for preview
     */
    public function getItem(){
        ...
        if(count($order) == 1){
            ...
            if(count($files) > 2){
                ...
                foreach($files as $file){
                    if($file != '..' && $file != '.' && substr($file,0,3) != 'tn_'){
                        $markup .=  '<a target="_blank" href="'.PAGE_URL.ORDERFILESPATH.$order['id'].'/'.$file.'">';
                        $markup .=  '<img src="'.PAGE_URL.ORDERFILESPATH.$order['id'].'/tn_'.$file.'" />';
                        $markup .=  '</a>';
                    }
                }
            ...
            $markup .= '<button class="ui-icon-close button right" onclick="cancelAction();">'.$this->text('close').'</button>
            <div class="clear"></div>';
    ...
    public function getNewItemForm(){
        if($_SESSION[PAGE_SESSIONID]['privileges']['orders'][1] == '0') die($this->text('dont_have_permission'));
    ...
    public function getEditItemForm(){
        if($_SESSION[PAGE_SESSIONID]['privileges']['orders'][2] == '0' && $_SESSION[PAGE_SESSIONID]['privileges']['orders'][3] == '0') die($this->text('dont_have_permission'));
    ...
    public function addItem(){
        ...
        //if user can change order ownership set values
        ...
        $returnvalue = array();
        if ($temp == 0 || $temp == 1)  {
            $returnvalue = array('state'=>'highlight','data'=> $this->text('h5'));
            $id = dibi::insertId();
            //creating directory for photos
            $temp = mkdir(FRONTEND_ABSDIRPATH.ORDERFILESPATH.$id,0777);
            if($temp === false) $returnvalue['data'] .= '<br />'.$this->text('e27');

            //sending mails for all about new order
            $this->data['id'] = $id;
            $this->sendAddEmail($this->data);
        }else {
            $returnvalue = array('state'=>'error','data'=> $this->text('e26'));
        }
    ...
    public function acquireItem(){
        ...
        $returnvalue = array();
        if ($temp == 0 || $temp == 1) {
            $returnvalue = array('state'=>'highlight','data'=> $this->text('h6'));
        }else {
            $returnvalue = array('state'=>'error','data'=> $this->text('e30'));
        }
    ...
    public function finishItem(){
        ...
        $returnvalue = array();
        if ($temp == 0 || $temp == 1) {
            $returnvalue = array('state'=>'highlight','data'=> $this->text('h8'));
        }else {
            $returnvalue = array('state'=>'error','data'=> $this->text('e33'));
        }
    ...
    public function deleteItem(){
        ...
        $returnvalue = array();
        if ($temp) {
            $returnvalue = array('state'=>'highlight','data'=> $this->text('h7'));
            ...
            $returnvalue = array('state'=>'error','data'=> $this->text('e32'));
        }
    ...
    private function getForm($formaction = 'add',$data = array()){
        if(isset($data['ukoncene']) && $data['ukoncene'] == '1'){
            ...
            $markup = '
                ...
                '.$this->text('t25').': <input class="date ui-autocomplete-input ui-widget-content ui-corner-all splittedright" type="text" name="datum_platnosti" id="datum_platnosti" value="'.((isset($data['datum_platnosti']))?date('d.m.Y',strtotime($data['datum_platnosti'])):date('d.m.Y',strtotime('+1 month'))).'" />
                </fieldset>';
                if($_SESSION[PAGE_SESSIONID]['privileges']['orders'][4] == '1' || ($formaction == 'update' && $_SESSION[PAGE_SESSIONID]['id'] == $data['vlastnik'])){
                    $markup .= '<fieldset>
                    <legend>'.$this->text('t26').':</legend>
                    <div class="help"><p>'.$this->text('h4').'</p></div>
                    '.$this->getUsersCombobox((isset($data['vlastnik']))?$data['vlastnik']:$_SESSION[PAGE_SESSIONID]['id']).'
                    </fieldset>';
                }
                if($formaction == 'update'){
                    ...
                    <fieldset>
                    <legend>'.$this->text('t23').':</legend>
                    <div class="help"><p>'.$this->text('h9').'</p></div>
                    ...
                    <input class="ui-autocomplete-input ui-widget-content ui-corner-all splittedright" type="text" name="sarza_setov" id="sarza_setov" value="'.((isset($data['sarza_setov']))?$data['sarza_setov']:'').'" /><label for="sarza_setov"> - '.$this->text('t34').'</label><br />
                    <input class="ui-autocomplete-input ui-widget-content ui-corner-all splittedright" type="text" name="mnozstvo_mat" id="mnozstvo_mat" value="'.((isset($data['mnozstvo_mat']))?$data['mnozstvo_mat']:'').'" /><label for="mnozstvo_mat"> - '.$this->text('t35').'</label><br /><br />
                    <button class="ui-icon-check button" onclick="finishItem('.$data['id'].');">'.$this->text('t20').'</button>
                    </fieldset>';
                }
    ...
    private function getUsersCombobox($activeuser = 'x',$groups = array()){
        $markup = '';
        $markup .= '<select name="vlastnik" id="vlastnik" class="ui-autocomplete-input ui-widget-content ui-corner-all">';
        $markup .= $this->getUsersComboboxElements($activeuser);
        $markup .= '</select>';
        return $markup;
    }
