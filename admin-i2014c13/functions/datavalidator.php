...
/**
 * Carries indataation about each of the data validations
 */
class ValidatorObj {
...
define("E_VAL_URL_CHECK_FAILED", "Please provide a valid url");
...
define("E_VAL_ALPHANUMERIC_CHECK_FAILED", "Please provide alphabetic, numeric or underslash character for %s");

/**
 * DataValidator: The main class that does all the data validations
 * */
class datavalidator {
    ...
    function ValidateData($data_variables) {
        $bret = true;
        ...
        foreach ($this->validator_array as $val_obj) {
            if (!$this->ValidateObject($val_obj, $data_variables, $error_string)) {
                $bret = false;
        ...
        if (true == $bret && count($this->custom_validators) > 0) {
            foreach ($this->custom_validators as $custom_val) {
                if (false == $custom_val->DoValidate($data_variables, $this->error_hash)) {
                    $bret = false;
                }
            }
        }
    ...
    function ValidateObject($validatorobj, $datavariables, &$error_string) {
        $bret = true;
        ...
        if (isset($datavariables[$validatorobj->variable_name])) {
            $input_value = $datavariables[$validatorobj->variable_name];
        }
    ...
    function validate_url($url) {
        return preg_match("/^(http|https|ftp|mailto):\/\/([\w-]+\.)+[\w-]+(\/[\w- .\/?%&=]*)?$/i", $url);
    }
    ...
    function ValidateCommand($command, $command_value, $input_value, &$default_error_message, $variable_name, $datavariables) {
        $bret = true;
        switch ($command) {
            case 'req': {
            ...
            case 'alphanumeric': {
                    $bret = $this->test_datatype($input_value, "[^A-Za-z0-9_]");
                    if (false == $bret) {
                        $default_error_message = sprintf(E_VAL_ALPHANUMERIC_CHECK_FAILED, $variable_name);
                    }
                    break;
                }
            case 'alpha_s': {
            ...
            case 'url': {
                    if (isset($input_value) && strlen($input_value) > 0) {
                        ...
                        if (false == $bret) {
                            $default_error_message = E_VAL_URL_CHECK_FAILED;
                        }
                    }
                    break;
                }
            ...
            case "eqelmnt": {
                    ...
                    if (isset($datavariables[$command_value]) &&
                            strcmp($input_value, $datavariables[$command_value]) == 0) {
                        $bret = true;
                    } else {
                        $bret = false;
